...
export default function GoldAssetDetailContent({ id, basePath = "/guide/gold" }: Props) {
  ...
  if (!asset) {
      return <div className="p-8 text-center text-slate-500">Asset Not Found: {id}</div>;
  }

  return (
    <div className="animate-fadeIn pb-20 max-w-6xl mx-auto px-4">

      <AdSenseBanner className="mb-6 min-h-[90px]" />

      <Link href={`${basePath}/market`} className="text-slate-400 hover:text-white text-sm mb-6 inline-block transition">
          {t('common.backToList')}
      </Link>
