...
const AssetCard = ({ id, item, type, t, language }: { id: string, item: any, type: string, t: (key: string) => string, language: string }) => {
    // Detect if this asset has a detail page (Dragons/Baron)
    const hasDetail = type === "objectives" && (id === "dragons" || id === "baron" || id === "herald" || id === "void_grubs");
    
    // Flatten nested dragon object if needed, or just skip header
    ...
    return (
        <div className="bg-slate-900/50 border border-slate-700/50 hover:border-primary/50 transition-all p-4 rounded-xl group relative overflow-visible">
             ...
             <div className="flex justify-between items-start mb-2 relative z-10">
                <div className="flex items-center gap-3">
                    {item.icon_url ? (
                        ...
                        // Fallback Generic Icons based on type if needed, or just layout spacing
                        <div className="w-10 h-10 flex items-center justify-center bg-slate-800 rounded-lg text-slate-500">
                             {/* Optional: Generic Icon based on category */}
             ...
             <div className="flex items-end justify-between relative z-10 mt-4">
                ...
                {hasDetail && (
                    <Link href={`/dashboard/economy/market/${id}`} className="text-xs bg-slate-700 text-slate-200 font-bold px-3 py-1.5 rounded hover:bg-slate-600 transition border border-white/10">
                        {t('common.readMore')}
                    </Link>
                )}
             </div>
...
const DragonsSection = ({ dragons, t, language }: { dragons: any, t: (key: string) => string, language: string }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {Object.entries(dragons).map(([key, dragon]: [string, any]) => (
            <Link key={key} href={`/dashboard/economy/market/dragon_${key}`} className="block h-full">
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-primary/20 hover:border-primary transition-all p-6 rounded-xl text-center relative group h-full flex flex-col items-center justify-center overflow-hidden">
                    ...
                    {/* Background glow effect based on dragon */}
...
export default function MarketListPage() {
  ...
  // Utility to categorize assets (uses translation keys)
  const categories = [
      ...
      <div className="animate-fadeIn pb-20 max-w-7xl mx-auto">
        ...
        <header className="mb-10">
                <div className="flex items-center gap-4 mb-4">
                    <Link href="/dashboard/economy" className="text-slate-400 hover:text-white transition">
        ...
        <section className="mb-12">
            ...
            <DragonsSection dragons={goldConstants.objectives.dragons} t={t} language={language} />
        </section>
            ...
            <section key={cat.id} className="mb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {Object.entries(cat.data).map(([key, item]) => (
                        <AssetCard key={key} id={cat.id === "objectives" ? key : ""} item={item} type={cat.id} t={t} language={language} />
                    ))}
                </div>
            </section>
        ))}
