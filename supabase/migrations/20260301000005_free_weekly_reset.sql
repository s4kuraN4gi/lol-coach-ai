-- Free users: change from monthly reset (3/month) to weekly reset (1/week)
-- This creates a healthier engagement loop and reduces the free-to-premium gap
...
BEGIN
  ...
  -- 2. Weekly credit replenishment (free users only, cap at 1)
  IF NOT rec.is_premium THEN
    ...
    ELSE
      ...
      IF time_diff_ms >= one_week_ms THEN
        ...
        IF COALESCE(rec.analysis_credits, 0) < 1 THEN
          new_credits := 1;
  ...
  -- 3. Analysis limit reset (weekly for ALL users)
  IF rec.weekly_reset_date IS NULL OR now_ts >= rec.weekly_reset_date THEN
    -- All users: reset weekly (next Monday)
